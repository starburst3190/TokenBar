...
pub fn parse_opencodereview_file(path: &Path) -> Vec<UnifiedMessage> {
    ...
    for line in BufReader::new(file).lines() {
        ...
        // Use the recorded end timestamp in the identity. The message timestamp
        // is start-anchored for sessionization, while a replay has the same
        // source record identity regardless of that derived anchor.
...
mod tests {
    ...
    fn deduplicates_identical_records() {
        let resp = llm_response("2026-01-15T10:00:05Z", "gpt-4o", 1000, 200, 0, 0);
        let content = format!("{}\n{}\n", resp, resp);
        let msgs = parse_events(&content);
        assert_eq!(msgs.len(), 1);
    }
    ...
    #[test]
    fn parses_multiple_responses_and_skips_zero_tokens() {
        let content = format!(
            "{}\n{}\n{}\n",
            llm_response(
                "2026-01-15T10:00:05Z",
            ...
            llm_response("2026-01-15T10:02:00Z", "gpt-4o", 0, 0, 0, 0),
        );
        let msgs = parse_events(&content);
    ...
    fn works_without_session_start() {
        let content = llm_response("2026-01-15T10:00:05Z", "gpt-4o", 1000, 200, 0, 0);
    ...
    fn start_anchor_uses_recorded_end_timestamp() {
        ...
        assert_eq!(msgs[0].timestamp, expected_end - 1500);
        assert_eq!(msgs[0].duration_ms, Some(1500));
    }

    #[test]
    fn missing_timestamp_does_not_anchor_from_file_fallback() {
        let dir = TempDir::new().unwrap();
        let path = dir.path().join("missing-time.jsonl");
        let content = r#"{"type":"llm_response","model":"gpt-4o","duration_ms":1500,"usage":{"prompt_tokens":100,"completion_tokens":50}}"#;
