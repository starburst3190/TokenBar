...
| Rolling tokscale alignment | M15-T records the complete post-M15-A 111-commit classification and the selected two-lane runtime DAG. The shared-parser critical path is M20 → M15-B → M16; M16 then unlocks the new-source lane M17 → M21 → M22 → M23 and the money/settings lane M18 → M25 → M24. M19-A can proceed after M15-T; M26 joins M23/M24/M19-A before one final Windows re-sync. Issue #45 is the designated public exact ledger but still needs the mandatory M15-T post-merge refresh before it is current; the private Project should contain executable milestones only | [issue #45](https://github.com/Nanako0129/TokenBar/issues/45), [`plans/tokscale-alignment.md`](plans/tokscale-alignment.md), [`vendor/README.md`](../../vendor/README.md) |
| OpenCode v2 M20 | Upstream PR #920 merged as `366ce643`; existing `opencode-next.db` discovery stays valid while the parser adds `session_message`, nested model/provider resolution, strict JSON-role behavior, and distinct-message dedup. It is the first runtime milestone after M15-T and rebuilds monolithic cache schema 29 → 30 | [upstream PR #920](https://github.com/junhoyeo/tokscale/pull/920), [`plans/tokscale-alignment.md`](plans/tokscale-alignment.md) |
