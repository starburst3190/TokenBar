...
|---|---|---|
| Provider-wide quota pace | Re-planned: the shipped Codex Weekly v2 evaluator is only a foundation. Every recurring Codex、Claude、Grok、Antigravity與Copilot quota card must acquire a trustworthy duration／account／window identity and explicit learning state before generic historical calculation; runtime work is not yet implemented | [`plans/provider-quota-pace.md`](plans/provider-quota-pace.md)、[`plans/codex-historical-pace-v2.md`](plans/codex-historical-pace-v2.md) |
