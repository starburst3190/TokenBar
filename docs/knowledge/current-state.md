...
| Vendor | Current main `aebbc371` includes selective tokscale alignment through existing-parser correctness M16; the clean upstream target is `366ce643`, covering 111 core commits classified as 66 already / 21 take / 0 adapt / 10 defer / 13 skip / 1 superseded; local cache schema is 31 |
...
| Rolling tokscale alignment | M16 merged as PR #67 at `aebbc371`, and issue #45 records the exact `66/21/0/10/13/1` public ledger with cache schema 31. M17 Grok precedence and M18 routed pricing are being prepared independently. M19-A is the active integration checkpoint: it takes only the Windows atomic-replacement hunk of `a87f0ab6`, moves that row to `ALREADY_VENDORED`, keeps schema 31, and produces `67/20/0/10/13/1`; TUI signal handling remains excluded. M26 joins M23/M24/M19-A before one final Windows re-sync. The private Project should contain executable milestones only | [issue #45](https://github.com/Nanako0129/TokenBar/issues/45), [PR #67](https://github.com/Nanako0129/TokenBar/pull/67), [`plans/tokscale-alignment.md`](plans/tokscale-alignment.md), [`vendor/README.md`](../../vendor/README.md) |
...
| Windows atomic replacement M19-A | Active integration checkpoint after M16. Only `a87f0ab6`'s `fs_atomic.rs` Windows hunk is selected: errors 5 and 32 retry for at most five attempts with exact 10/20/30/40 ms backoff; non-transient errors and non-Windows rename behavior remain single-attempt. Injected replacement/sleep fixtures pin both retryable codes, success, terminal failure, immediate failure, and exact timing. Cache schema remains 31; upstream TUI signal behavior is excluded | [upstream PR #906](https://github.com/junhoyeo/tokscale/pull/906), [`plans/tokscale-alignment.md`](plans/tokscale-alignment.md), [`vendor/README.md`](../../vendor/README.md) |
