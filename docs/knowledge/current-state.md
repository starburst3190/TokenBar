...
| Vendor | Current main `f5773ea0` includes selective tokscale alignment through Kiro structured sessions M15-B; the clean upstream target is `366ce643`, covering 111 core commits classified as 62 already / 26 take / 0 adapt / 9 defer / 13 skip / 1 superseded; local cache schema is 30 |
...
| Rolling tokscale alignment | M15-B merged as PR #66 at `f5773ea0`, and issue #45 now records the exact `62/26/0/9/13/1` public ledger with cache schema 30. M16 is the active implementation checkpoint: it moves `6899ea03`, `b59979c5`, `9155018c`, and `18cd13cc` to `ALREADY_VENDORED`, moves mixed `34cfbb50` to `DEFER` after taking only provider hardening, keeps mixed `b64d861e` in `TAKE` after its Jcode hunk, advances cache schema to 31, and produces `66/21/0/10/13/1`. M16 then unlocks the new-source lane M17 → M21 → M22 → M23 and the money/settings lane M18 → M25 → M24. M19-A prep is complete in isolation but awaits its own integration/docs lock; M26 joins M23/M24/M19-A before one final Windows re-sync. The private Project should contain executable milestones only | [issue #45](https://github.com/Nanako0129/TokenBar/issues/45), [PR #66](https://github.com/Nanako0129/TokenBar/pull/66), [`plans/tokscale-alignment.md`](plans/tokscale-alignment.md), [`vendor/README.md`](../../vendor/README.md) |
...
| Existing-parser correctness M16 | Active implementation checkpoint after M15-B. Selected scope is Codex legacy replay boundaries and lenient start metadata, Claude bare-transcript explicit-token behavior, start-anchored duration timestamps across Codex/Claude/Copilot/Jcode, monotonic duplicate merging for Claude/Copilot, provider identity recovery, and current Antigravity aliases. The 9Router bridge remains excluded. Existing-source serialized output advances monolithic cache schema 30 → 31; hermetic parser fixtures and a same-fingerprint schema-30 Jcode entry prove old completion anchoring differs from the rebuilt start anchor, with cold/warm materialized, streaming, count, model, monthly, hourly, and Agents parity | [upstream PR #896](https://github.com/junhoyeo/tokscale/pull/896), [PR #892](https://github.com/junhoyeo/tokscale/pull/892), [PR #890](https://github.com/junhoyeo/tokscale/pull/890), [PR #891](https://github.com/junhoyeo/tokscale/pull/891), [PR #887](https://github.com/junhoyeo/tokscale/pull/887), [PR #898](https://github.com/junhoyeo/tokscale/pull/898), [`plans/tokscale-alignment.md`](plans/tokscale-alignment.md), [`vendor/README.md`](../../vendor/README.md) |
