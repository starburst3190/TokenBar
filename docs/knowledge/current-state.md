...
| Vendor | Current main `1bc2fa76` includes selective tokscale alignment through OpenCode v2 M20; the clean upstream target is `366ce643`, covering 111 core commits classified as 60 already / 28 take / 0 adapt / 9 defer / 13 skip / 1 superseded; local cache schema is 30 |
...
| Rolling tokscale alignment | M20 merged as PR #65 at `1bc2fa76`, and issue #45 now records the exact `60/28/0/9/13/1` public ledger with cache schema 30. M15-B is the active implementation checkpoint; after merge it moves `405ded4a` and `315549b4` to `ALREADY_VENDORED`, producing `62/26/0/9/13/1`, while mixed `b64d861e` remains `TAKE`. M16 is next on the shared-parser critical path and then unlocks the new-source lane M17 → M21 → M22 → M23 and the money/settings lane M18 → M25 → M24. M19-A prep is complete in isolation but awaits its own integration/docs lock; M26 joins M23/M24/M19-A before one final Windows re-sync. The private Project should contain executable milestones only | [issue #45](https://github.com/Nanako0129/TokenBar/issues/45), [PR #65](https://github.com/Nanako0129/TokenBar/pull/65), [`plans/tokscale-alignment.md`](plans/tokscale-alignment.md), [`vendor/README.md`](../../vendor/README.md) |
...
| Kiro structured sessions M15-B | Active implementation checkpoint after M20. The scanner adds only `~/.kiro/sessions/<workspace>/sess_*/session.json` anchors; structured turns read sibling `messages.jsonl`, estimate context/output, preserve flat fallback, and back-calculate missing prompt anchors from turn end and elapsed time. One related-file helper already feeds specialized fingerprinting, both cache lanes, latest mtime, and fail-open pruning. IDE-only model fallback is `auto`; existing CLI/SQLite fallback stays `unknown`, so schema 30 remains valid. Hermetic coverage exercises absent/created/rewritten siblings, warm rebuild, M15-A coexistence, materialized/streaming/count, and model/monthly/hourly/Agents parity | [upstream PR #836](https://github.com/junhoyeo/tokscale/pull/836), [PR #847](https://github.com/junhoyeo/tokscale/pull/847), [`plans/tokscale-alignment.md`](plans/tokscale-alignment.md), [`vendor/README.md`](../../vendor/README.md) |
