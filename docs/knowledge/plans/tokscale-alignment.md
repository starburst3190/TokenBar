...
| M17 | M16 | Active implementation checkpoint: discover the self-contained global unified log, cache raw unified/legacy sources independently, and select unified-covered sessions exactly once across materialized, shipping streaming, count, and report lanes; a topology-sensitive FFI graph/live-tail token restores legacy fallback when a non-max unified source disappears | Keep schema 31 |
