...
History load separates structural corruption from clock disagreement. Schema version, series ordering, sample validity, per-cycle caps, rollover coherence and activity-behind-its-own-samples remain corruption: the file is quarantined and the store restarts empty. A persisted `last_activity_at` that merely leads the reading transaction is a clock event rather than corruption — the wall clock steps backwards on an NTP correction or sleep/wake, while `observation_now` is captured before a provider run that can outlive the step — so a lead within `CLOCK_SKEW_TOLERANCE_SECONDS` is absorbed and nothing is discarded. Past that tolerance only the unverifiable series is dropped; the file is still quarantined as evidence, every other provider keeps its samples, and the repaired store is written back even when the transaction body changes nothing. The post-body validation shares the same ceiling as the load, so an admitted store cannot fail on the way out.
