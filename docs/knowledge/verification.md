...
| Quota provider toggles | `crates/tb_core_ffi/src/disabled_providers.rs` covers the registry itself: known ids register, a repeat folds (it is a set, unlike the Claude config dirs where a repeat means two cards writing one series), a wrong-case or empty id is rejected and reported rather than stored, `[]` is full-replace back to everything enabled, and malformed JSON leaves the registry untouched. The claim that actually matters is in `agent_usage.rs`: `every_disabled_provider_is_skipped_before_its_future_is_built` disables all five and asserts the payload carries no cards. It is hermetic precisely because it is total — a provider still being fetched would need credentials and a socket — and its timing is the evidence: it completes in 0.00s, while the mutation that ignores the registry takes 8.10s and returns three cards. Mutation-verified by replacing the registry lookup with `true`. |
| Antigravity CLI cache | `agent_antigravity.rs` tests the decision function, not the spawn: empty cache forces the caller to block (the one cold start per process), a fresh entry serves without arming the in-flight guard, a stale entry serves the old value and hands out exactly one refresh while three later callers get `Serve`, a cached failure expires sooner than a success **asserted against the same instant** so collapsing the two TTLs fails, a cached failure surfaces as a failure rather than an empty card, and the in-flight flag is released by `Drop` even when the refresh panics — a leaked flag is permanent and would freeze the card on one value for the process lifetime. Mutation-verified three ways, each red-then-green: collapsing the negative TTL into the positive one, removing the `compare_exchange` single-flight guard, and emptying the `Drop` body. |
...
Windows port（[Nanako0129/TokenBar-Windows](https://github.com/Nanako0129/TokenBar-Windows)）的 C# `TokenBar.Core` 是 `Sources/TokenBarCore` 的逐檔移植。Native reviewed pin 為 `02c883d2c0792cc746d7469939eb4dd8775f698c`；Windows current pin 與 consumer state 由 Windows repo 自己擁有，Native 不重述。這次 consumer 是 pin-only：`crates/tb_core_ffi` 零改動（新欄位的填值在前次推進就做完了），也沒有 app-owned C ABI 變更隨行，`ctb.h` 簽名不變，故 Windows 不是本次簽名規則下的必須通知消費者。Windows 推進自己的 pin 時仍需要那一行填值，因為它的 `TokenBreakdown` literal 同樣是窮盡的。（本次 `65511b8` → `02c883d` delta 為 4 個 engine commit；前次 `044153f` → `65511b8` 的 4 commit、再前次 `ffe5a2d` → `044153f` 的 7 commit、更前次 `47c7241` → `ffe5a2d` 的 10 commit、再更前次 `434b95ff` → `47c7241` 的 52 commit。`crates/tb_core_ffi` 只在前次動過一行。）LocalOnly、CostCoverage、embedded-cost 與 partial-estimation 語意尚未宣稱已經 C ABI 抵達 Swift。global cache format 與 serialized layout 本次**不變**（`CACHE_FORMAT_VERSION` 維持 4）；變的是 `parser_version(Claude)` 3→4，只讓 Claude shard 重新 parse。前次推進才是 format bump 那一次。本次 pin 帶進 **#287 的 1h cache write 計價修正**（engine PR #25）。Anthropic 對 prompt cache 寫入收兩種費率——5 分鐘 TTL 是 base input 的 1.25 倍、1 小時是 2 倍——而 parser 只讀兩者之和 `cache_creation_input_tokens`，全部按較便宜的 5m 費率計。現在 parser 讀巢狀的 `cache_creation.ephemeral_1h_input_tokens`，計價時把 1h 從 cache-write 桶**扣除**後以 `2 × input_cost_per_token` 重計。`parser_version(Claude)` 3→4，這一步不可省：`get` 只要版本相符就當快取命中，指紋未變的 transcript 永遠不會被重新 parse，修正對所有既有使用者將完全無效。此 bump 落在可遷移區間，retained-only turn 由前次 pin 帶進的 migration 保住。
