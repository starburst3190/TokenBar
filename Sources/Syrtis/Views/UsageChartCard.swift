    ...
    var body: some View {
        ...
        DashCard(
            ...
            if is3D {
                ...
                legendView(legend)
                ScrollingBarChart(
                    bars: bars, metric: metric, regrowKey: "\(metricRaw)|\(stackByRaw)")
            }
        }
...
private struct ScrollingBarChart: View {
    ...
    /// Changes when the bars are re-derived (metric or stacking), which is what
    /// the glass panel's regrow animation keys on.
    let regrowKey: String
