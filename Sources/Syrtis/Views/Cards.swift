...
/// Floating hover-tooltip chrome for every host — the popover, the macOS 27
/// Liquid Glass panel, and Settings (`tooltipSurface()` in
/// GlassPanelPresenter.swift). On macOS 26+ it's real Liquid Glass so the
...
struct PopoverTooltipSurface: ViewModifier {
...
struct SegmentedPicker<Value: Hashable>: View {
    ...
    private func label(_ text: String, on: Bool) -> some View {
        Text(text.localized)
            ...
            .foregroundStyle(on ? AnyShapeStyle(.primary) : AnyShapeStyle(.secondaryAdaptive))
